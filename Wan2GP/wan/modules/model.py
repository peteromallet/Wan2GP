...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # This is the original x that we will add hints to later
        x_after_main_processing = self._core_forward(x, e, grid_sizes, freqs, context, cam_emb, block_mask, audio_proj, audio_context_lens, audio_scale)

        total_added_hint = torch.zeros_like(x_after_main_processing)
        if self.block_id is not None and vace_streams_states is not None and max_steps > 0:
            current_step_percentage = current_step / max_steps
            
            kwargs_for_vace_sub_block = { 
                "e" : e,
            ...
            for vace_stream in vace_streams_states:
                is_active = vace_stream['start_percent'] <= current_step_percentage < vace_stream['end_percent']
                if is_active:
                    current_processing_input = vace_stream['current_latent_state']
                    
                    # For block 0, the main diffusion latent 'x' is mixed into the VACE stream.
                    # For subsequent blocks, the VACE stream evolves independently.
                    main_diffusion_latent_for_vace_sub_block = x if self.block_id == 0 else None
                    
                    returned_hint_for_x, next_stream_state = self.vace(
                        current_processing_input,
                        main_diffusion_latent_for_vace_sub_block,
                        **kwargs_for_vace_sub_block
                    )
                    
                    vace_stream['current_latent_state'] = next_stream_state
                    
                    # Use per-step scale if available, otherwise default to 1.0
                    if vace_stream.get('scale_per_step_array') is not None:
                        current_scale = vace_stream['scale_per_step_array'][current_step]
                        total_added_hint.add_(returned_hint_for_x, alpha=current_scale)

        x_after_main_processing.add_(total_added_hint)
        return x_after_main_processing
...
class VaceWanAttentionBlock(WanAttentionBlock):
    ...
    def forward(self, current_processing_input, main_diffusion_latent, **kwargs):
        c = current_processing_input
        
        if self.block_id == 0:
            c = self.before_proj(c)
            if main_diffusion_latent is not None:
                c += main_diffusion_latent

        # Process the VACE latent through the attention block's core logic
        next_stream_state = self._core_forward(
            c,
            e=kwargs['e'],
            grid_sizes=kwargs['grid_sizes'],
            freqs=kwargs['freqs'],
            context=kwargs['context'],
            cam_emb=kwargs.get('cam_emb'),
            block_mask=kwargs.get('block_mask'),
            audio_proj=kwargs.get('audio_proj'),
            audio_context_lens=kwargs.get('audio_context_lens'),
            audio_scale=kwargs.get('audio_scale')
        )
        
        # Project the result to get the hint to be added to the main diffusion latent
        returned_hint_for_x = self.after_proj(next_stream_state)
        
        return returned_hint_for_x, next_stream_state
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if vace_configs:
            vace_streams_states = []
            for config in vace_configs:
                # Assuming batch size of 1 for now, taking the first element of raw_vace_latent
                # which is a list. This matches the old logic.
                patched_latent = [self.vace_patch_embedding(u.to(self.vace_patch_embedding.weight.dtype).unsqueeze(0)) for u in config['raw_vace_latent']]
                patched_latent = [u.flatten(2).transpose(1, 2) for u in patched_latent]
                
                vace_streams_states.append({
                    'id': config.get('id'),
                    'current_latent_state': patched_latent[0],
                    'scale_per_step_array': config.get('scale_per_step_array'),
                    'start_percent': config.get('start_percent', 0.0),
                    'end_percent': config.get('end_percent', 1.0),
                    'output_hint_for_x': None,
                })
        ...
        if not should_calc:
            ...
            for block_idx, block in enumerate(self.blocks):
                ...
                # Add vace_streams_states to kwargs to be passed to the block
                kwargs['vace_streams_states'] = vace_streams_states
                ...
                if (x_id != 0 or joint_pass) and slg_layers is not None and block_idx in slg_layers:
                    ...
                    # The block will now receive vace_streams_states and update it in place.
                    for i, (x, context, audio_scale) in enumerate(zip(x_list, context_list, audio_scale_list)):
                        x_list[i] = block(x, context = context, audio_scale= audio_scale, e= e0, **kwargs)
                        del x
                    del context
