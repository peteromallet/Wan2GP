...
class WanT2V:
    ...
    def generate(self,
        ...
        Args:
            ...
            vace_configs (`list[dict]`, *optional*):
                A list of VACE conditioning configurations. Each configuration is a dictionary:
                {
                    'id': str,
                    'frames': list[Tensor],
                    'masks': list[Tensor],
                    'ref_images': list[list[Image]],
                    'scale_per_step_array': np.array,
        ...
        """
        # preprocess
        vace = "Vace" in model_filename or vace_configs is not None
        ...
        if vace :
            prepared_vace_data_for_model = []
            z0 = None
            if vace_configs:
                for vace_config in vace_configs:
                    input_frames = vace_config.get('frames')
                    input_masks = vace_config.get('masks')
                    input_ref_images = vace_config.get('ref_images')

                    input_frames = [u.to(self.device) for u in input_frames] if input_frames else [None]
                    input_ref_images = [None if u is None else [v.to(self.device) for v in u] for u in input_ref_images] if input_ref_images else [None]
                    input_masks = [u.to(self.device) for u in input_masks] if input_masks else [None]

                    # Note: overlapped_latents is applied to all streams for now.
                    current_z0 = self.vace_encode_frames(input_frames, input_ref_images, masks=input_masks, tile_size=VAE_tile_size, overlapped_latents=overlapped_latents)
                    m0 = self.vace_encode_masks(input_masks, input_ref_images)
                    z = self.vace_latent(current_z0, m0)

                    if z0 is None:
                        z0 = current_z0

                    prepared_vace_data_for_model.append({
                        'id': vace_config.get('id'),
                        'raw_vace_latent': z,
                        'scale_per_step_array': vace_config.get('scale_per_step_array'),
                        'start_percent': vace_config.get('start_percent', 0.0),
                        'end_percent': vace_config.get('end_percent', 1.0)
                    })
            
            if z0 is not None:
                target_shape = list(z0[0].shape)
                target_shape[0] = int(target_shape[0] / 2)
            else: # Should not happen if vace is True and vace_configs is provided
                F = frame_num
        ...
        if vace:
            # The new structure for vace_context will be handled in Phase 2 inside the model.
            # For now, we prepare it to be passed.
            kwargs.update({'vace_configs': prepared_vace_data_for_model})
            if overlapped_latents != None and len(vace_configs) > 0:
                # This part needs careful review in Phase 2.
                # It assumes properties from the first config apply to all,
                # which might not be what we want long-term.
                input_ref_images_count = len(vace_configs[0].get('ref_images', [None])[0]) if vace_configs[0].get('ref_images') and vace_configs[0].get('ref_images')[0] else 0
                overlapped_latents_size = overlapped_latents.shape[1] + 1
                # This z is from the last item in the loop, which is incorrect for multi-stream.
                # This part of the logic will be moved/re-implemented in Phase 2 inside the model.
                # For now, we are just passing the configs.
                # z_reactive = [zz[0:16, 0:overlapped_latents_size + input_ref_images_count].clone() for zz in z]
        ...
        for i, t in enumerate(tqdm(timesteps)):
            ...
            timestep = [t]
            if overlapped_latents != None and vace:
                # This logic is complex and tied to a single vace stream.
                # It will be disabled for now and addressed in Phase 2.
                pass
                # overlap_noise_factor = overlap_noise / 1000
                # latent_noise_factor = t / 1000
                # for zz, zz_r, ll in zip(z, z_reactive, [latents]):
                #     pass
                #     zz[0:16, input_ref_images_count:overlapped_latents_size + input_ref_images_count] = zz_r[:, input_ref_images_count:] * (1.0 - overlap_noise_factor) + torch.randn_like(zz_r[:, input_ref_images_count:]) * overlap_noise_factor
                #     ll[:, 0:overlapped_latents_size + input_ref_images_count] = zz_r * (1.0 - latent_noise_factor) + torch.randn_like(zz_r) * latent_noise_factor
        ...
        if input_frames == None:
            ...
            videos = self.decode_latent(x0, vace_configs[0]['ref_images'] if vace_configs else None, VAE_tile_size)
        if return_latent_slice != None:
